...
{
public:
  explicit LineArray(const uint8_t deviceAddress, TwoWire *wire = &Wire);
  ...
  void    write4(const uint8_t value);
  void    write(const uint8_t pin, const uint8_t value);
  uint8_t valueOut() const { return _dataOut; }

  //added 0.1.07/08 Septillion
  inline uint8_t readPin4()  { return LineArray::readPin4(_buttonMask); }
  uint8_t        readPin4(const uint8_t mask);
  uint8_t        readPin(const uint8_t pin);
  inline void    setPinMask(const uint8_t mask) { _buttonMask = mask; };
